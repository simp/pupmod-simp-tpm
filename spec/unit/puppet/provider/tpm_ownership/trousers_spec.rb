...
describe Puppet::Type.type(:tpm_ownership).provider(:trousers) do
  ...
  let(:tpm_fact) do
    JSON.parse(File.read(File.expand_path('spec/files/tpm_fact.json')))
  end
  ...
  describe 'dump_owner_pass' do
    ...
    context 'with advanced_facts => true' do
      it 'drops off the password file' do
        expect(provider.dump_owner_pass(loc)).to match(%r{twentycharacters0000})
  ...
  describe 'tpm_takeownership' do
    let(:stdin) do
      [
        [ %r{owner password}i, resource[:owner_pass] ],
        [ %r{Confirm password}i, resource[:owner_pass] ],
        [ %r{SRK password}i,     resource[:srk_pass]   ],
        [ %r{Confirm password}i, resource[:srk_pass]   ],
      ]
    end
