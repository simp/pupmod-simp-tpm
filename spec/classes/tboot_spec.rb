...
describe 'tpm::tboot' do
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      let(:facts) do
        os_facts[:augeasprovider_grub_version] = if os_facts[:os][:release][:major] >= '7'
                                                   2
                                                 else
                                                   1
                                                 end
      ...
      context 'default options with tboot version known' do
        ...
        if os_facts[:os][:release][:major].to_i == 7
          ...
          it 'locks the default packages' do
          ...
          it { is_expected.to compile.and_raise_error(%r{does not currently support Grub 0.99-1.0}) }
        end
      end
      ...
      context 'with tboot version < 1/9/7 and create_policy set to true' do
        ...
        it { is_expected.to compile.and_raise_error(%r{version of tboot installed must be 1.9.7 or greater to create a policy}) }
      end
      ...
      context 'different grub2 parameters, create_policy true and lock_kernel_packages set to false' do
        ...
        if os_facts[:os][:release][:major].to_i == 7
          it {
            is_expected.to contain_file('/etc/default/grub-tboot').with_content(<<~EOF.chomp)
