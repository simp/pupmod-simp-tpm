...
class Facter::TPM::Util
    ...
    begin
      if owner_pass == 'well-known'
        ...
        PTY.spawn(cmd) do |r, w, pid|
          w.sync = true
          begin
            r.expect(%r{owner password}i, 15) { |_s| w.puts owner_pass }
  ...
  def version
    ...
    if (cmd_out == '') || cmd_out.nil?
      output['_status'] = 'Trousers is not running'
      return output
    else
      version = YAML.safe_load(cmd_out)

      # Format keys in a way that Facter will like
      begin
        output = Hash[version.flatten[1].map { |k, v| [k.downcase.tr(' ', '_'), v] }]
      rescue NoMethodError
        version.delete_if { |k, _y| k.include?('Version Info') }
        output = Hash[version.map { |k, v| [k.downcase.tr(' ', '_'), v] }]
      end
  ...
  def status
    ...
    files.each do |file|
      ...
      out[File.basename(file)] = if raw.is_a? Hash
                                   Hash[raw.map { |k, v| [k.downcase.tr(' ', '_'), v] }]
                                 else
                                   raw
                                 end
    end
  ...
  def pubek(status)
    ...
    if raw.nil? || (raw == '')
      ...
      pubek = YAML.safe_load(raw.gsub("\t", ' ' * 4).split("\n").drop(1).join("\n"))
      pubek['Public Key'].to_s.delete!(' ')
      pubek['raw'] = raw
      out.merge!(Hash[pubek.map { |k, v| [k.downcase.tr(' ', '_'), v] }])
